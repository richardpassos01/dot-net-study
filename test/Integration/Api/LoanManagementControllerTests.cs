...
public class LoanManagementControllerTests
{
    ...
    public async Task GetLoan_ShouldReturnLoanWithCorrectProperties()
    {
        ...
        using var context = new RelationalDbContext(options);

        context.Database.EnsureCreated();

        var loanApplicationRepository = new LoanApplicationRepository(context);
        var getLoan = new GetLoan(loanManagementAdapter);
        var loanOrigination = new LoanOrigination(loanManagementAdapter, loanApplicationRepository);
